...
vector<int> townhall_white_x = {0,2,4,6,8}; // config
vector<int> townhall_white_y = {0,0,0,0,0};
vector<int> townhall_black_x = {1,3,5,7,9};
vector<int> townhall_black_y = {9,9,9,9,9};
...
class board{
    ...
    bmove AplhaBetaSearchHeight(bool repeat)
    {
        ...
        if (players_remaning < 25)
        {
            min_height = 4;
        }
        else if (players_remaning < 22)
        {
            min_height = 5;
        }
        else if (players_remaning < 20)
        {
            min_height = 6;
        }
        ...
        {
            ans = MaxValueHeight(0, (-1)*FLT_MAX, FLT_MAX,1,repeat,4,4);
            cerr << "3" << endl;
        } 
        cerr << "Min Height : " << min_height << " and no of moves P =  " << this->getAllValidMoves(repeat).size() << endl;
    ...
    bool terminal(){
        if(this->c_board.num_black_townhalls < 4 || this-> c_board.num_white_townhalls < 4 || this->c_board.white_players.size() == 0 || this->c_board.black_players.size() == 0) // config
        {
            // cerr << " g1 " << this->c_board.num_black_townhalls << " " << this->c_board.num_white_townhalls << " " << this->c_board.white_players.size() << " " << this->c_board.black_players.size() <<  endl;
            return true;
        }
...
int main(int argc, char const *argv[])
{
    ...
    string a;
    getline(cin,a);
    vector<int> white_x = {1,1,1,3,3,3,5,5,5,7,7,7,9,9,9}; // config
    vector<int> white_y = {0,1,2,0,1,2,0,1,2,0,1,2,0,1,2};
    vector<int> black_x = {0,0,0,2,2,2,4,4,4,6,6,6,8,8,8};
    vector<int> black_y = {7,8,9,7,8,9,7,8,9,7,8,9,7,8,9};

    int intial_number_players = 15; // config
    ...
    b_w.num_black_townhalls = 5; // config
    b_w.num_white_townhalls = 5; // config
